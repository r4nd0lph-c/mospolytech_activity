...
/*$(document).ready(function(display_type_id) {
    // Проверяем, есть ли параметры запроса в URL
    var urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('name') && urlParams.has('group') && urlParams.has('dates')) {
        var name = urlParams.get('name');
        var group = urlParams.get('group');
        var dates = urlParams.getAll('dates');
        
        // Выполняем запрос расписания
        get_schedule({ "name": name, "group": group }, dates, display_type_id);
    }
});*/

function get_schedule(student, dates, display_type_id, post_guid) {
    /* student = {"name": "...", "group": "..."} dates = ["dd.mm.yyyy", ...] */
    $.ajax({
        type: "POST",
        url: "get_schedule/" + post_guid + "/",
        data: {
            guid: student["guid"],
            name: student["name"],
            group: student["group"],
        ...
        success: function (data) {
            ...
            // Обновляем адресную строку браузера и сохраняем текущее состояние
...
function updateUrl(selected_studentid, display_type_id, date) {
    let currentUrl = new URL(window.location.href);
    currentUrl.searchParams.set('studentid', selected_studentid);
    currentUrl.searchParams.set('display_type', display_type_id);
...
function checkURLParametersAndLoad() {
    ...
    const display_type_id = urlParams.get('display_type');
    const date = urlParams.get('date');
    if (studentId && display_type_id && date) {
        loadStudentURL(studentId, display_type_id, date);
    }
}


// Функция для загрузки данных из URL
function loadStudentURL(studentId, display_type_id, date) {
    $.ajax({
        url: `/${studentId}/${display_type_id}/${date}/`, 
        method: 'POST',
        data: { studentId: studentId,
                csrfmiddlewaretoken: CSRF_TOKEN }, // Передаем ID студента на сервер
        success: function(response) {
            if (response.student) {
                let student = response.student;
                document.querySelector("[name='student']").value = student.name;
                console.log(student);
                console.log(display_type_id);
                console.log(date);
            } else {
                console.error('Ошибка при получении данных о студенте');
            }
        },
...
checkURLParametersAndLoad();
    // Загружаем данные о студенте при загрузке страницы, если есть ID студента в адресной строке
...
$(document).ready(function () {
    ...
    if (formId === 'schedule-form') {
        ...
        button_search.onclick = function (e) {
            ...
            let selected_studentid = $(":input[name$=student]").select2("data")[0].id;
            ...
            if (display_type_id === "day") {
                ...
                updateUrl(selected_studentid, display_type_id, dates)
            } else if (display_type_id === "week") {
